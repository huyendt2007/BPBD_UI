...
| Mã khách hàng | String(50) | Không | Trống | Control UI: Input text.<br>- Placeholder: "Nhập mã khách hàng...".<br>- Tìm kiếm chính xác hoặc gần đúng theo Mã khách hàng của người yêu cầu. |
...
| Cán bộ xử lý | Enum(String(255)) | Không | Tất cả | Control UI: Combobox.<br>- Chọn Chuyên viên/Cán bộ đã xử lý hoặc trình ký hồ sơ, hoặc chọn "Tất cả". |
...
| Bảng danh sách hồ sơ | Text(1000) | Không | 20 bản ghi/trang | \- Chỉ hiển thị hồ sơ Yêu cầu cung cấp thông tin ở trạng thái "Chờ duyệt" thuộc phạm vi xử lý của Lãnh đạo đăng nhập.<br>\- Sắp xếp mặc định theo Thời điểm trình ký tăng dần để ưu tiên xử lý hồ sơ được trình trước.<br>\- Click trực tiếp vào dòng dữ liệu để mở **UC-CCTT-LD.MH02 - Chi tiết hồ sơ chờ duyệt**.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001].|
...
|  |  |  | TH Hợp lệ: Mở **UC-CCTT-LD.MH03 - Popup Từ chối yêu cầu cung cấp thông tin**. |
| 5 | Xem chi tiết | Nút | Mở **UC-CCTT-LD.MH02 - Chi tiết hồ sơ chờ duyệt**. |
| 6 | Click dòng dữ liệu | Row Click | Mở **UC-CCTT-LD.MH02 - Chi tiết hồ sơ chờ duyệt** nếu hồ sơ còn thuộc phạm vi duyệt của Lãnh đạo. |
...
| Nguồn tiếp nhận | Enum(String(50)) | Có | Theo hồ sơ | \- Chỉ đọc.<br>\- Hiển thị nguồn tiếp nhận theo nhóm nghiệp vụ: "Trực tuyến" hoặc "Trực tiếp". |
...
| **III. Kết quả tra cứu** | | | |<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001]. |
| Bảng danh sách kết quả tra cứu | Text(4000) | Không | Theo kết quả tra cứu | \- Chỉ đọc. Hiển thị kết quả tra cứu đã được Cán bộ lưu và khóa khi trình ký.<br>\- Nội dung hiển thị phải giống hệt **VI. Kết quả cung cấp thông tin có xác nhận của cơ quan đăng ký** tại màn Xem chi tiết hồ sơ của Website Khách hàng trong tài liệu [SRS yêu cầu cung cấp thông tin.md](../../01_Website_Khach_hang/SRS%20y%C3%AAu%20c%E1%BA%A7u%20cung%20c%E1%BA%A5p%20th%C3%B4ng%20tin.md#vi-ket-qua-cung-cap-thong-tin-co-xac-nhan-cua-co-quan-dang-ky).<br>\- Nếu phiên tra cứu không có dữ liệu, hiển thị [MSG-WRN-CCTT-001] dạng Inline ngay trong khu vực kết quả tra cứu, không hiển thị Toast.<br>\- Dữ liệu nguồn vẫn tuân thủ [BR-CCTT-002].<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001].|
...
|  |  |  | TH2 (Lãnh đạo không có quyền ký hồ sơ): Vi phạm [BR-CCTT-007], hiển thị [MSG-ERR-CCTT-007], không mở popup từ chối. |
|  |  |  | TH Hợp lệ: Mở **UC-CCTT-LD.MH03 - Popup Từ chối yêu cầu cung cấp thông tin**. |
| 5 | Quay lại | Nút | Quay về **UC-CCTT-LD.MH01 - Danh sách yêu cầu cung cấp thông tin chờ duyệt**. |
...
| Tiêu chí yêu cầu cung cấp thông tin | Enum(String(50)) | Không | Tất cả | \- Lọc theo tiêu chí người yêu cầu hoặc Cán bộ tiếp nhận đã chọn khi lập yêu cầu.<br>\- Các giá trị gồm:<br>\+ Tất cả<br>\+ Số đăng ký<br>\+ Bên bảo đảm<br>\+ Số khung |
...
| **II. Bảng danh sách hồ sơ** | | | |<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001].|
| Bảng danh sách hồ sơ | Text(1000) | Không | 20 bản ghi/trang | \- Chỉ hiển thị hồ sơ Yêu cầu cung cấp thông tin ở trạng thái "Chờ ký" thuộc phạm vi ký của Lãnh đạo đăng nhập.<br>\- Sắp xếp mặc định theo Thời điểm chuyển sang "Chờ ký" tăng dần.<br>\- Cho phép Lãnh đạo tích chọn một hoặc nhiều hồ sơ trên danh sách để ký số cùng một lúc.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001].|
...
| Nguồn tiếp nhận | Enum(String(50)) | Có | Theo hồ sơ | Chỉ đọc. Hiển thị nguồn tiếp nhận theo nhóm nghiệp vụ: "Trực tuyến" hoặc "Trực tiếp". Giá trị "Trực tiếp" xác định hồ sơ giấy được phép trả lại theo [BR-CCTT-009]. |
...
| Thao tác | String(255) | Không | Theo quyền | Hiển thị các thao tác:<br>+ "Xem chi tiết": áp dụng mọi hồ sơ thuộc quyền.<br>+ "Ký số": áp dụng hồ sơ đủ điều kiện ký.<br>+ "Từ chối": chỉ hiển thị với hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp".<br>+ "Trả lại": chỉ hiển thị với hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp". |
...
|  |  |  | TH Hợp lệ: Mở **UC-CCTT-LD.MH05 - Popup Ký số yêu cầu cung cấp thông tin** và truyền toàn bộ danh sách hồ sơ đã chọn sang popup để ký số cùng một lúc. |
| 4 | Từ chối | Nút | TH1 (Hồ sơ không phải hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp"): Vi phạm [BR-CCTT-007], không hiển thị/không cho phép thao tác. |
|  |  |  | TH2 (Hồ sơ không còn ở trạng thái "Chờ ký"): Vi phạm [BR-CCTT-007], hiển thị [MSG-ERR-CCTT-013], không cho phép từ chối. |
|  |  |  | TH Hợp lệ: Mở **UC-CCTT-LD.MH06 - Popup Từ chối/Trả lại hồ sơ giấy yêu cầu cung cấp thông tin** với Loại xử lý mặc định là "Từ chối". |
| 5 | Trả lại | Nút | TH1 (Hồ sơ không phải hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp"): Vi phạm [BR-CCTT-009], không hiển thị/không cho phép thao tác. |
|  |  |  | TH2 (Hồ sơ không còn ở trạng thái "Chờ ký"): Vi phạm [BR-CCTT-009], hiển thị [MSG-ERR-CCTT-013], không cho phép trả lại. |
|  |  |  | TH Hợp lệ: Mở **UC-CCTT-LD.MH06 - Popup Từ chối/Trả lại hồ sơ giấy yêu cầu cung cấp thông tin** với Loại xử lý mặc định là "Trả lại". |
...
| Nguồn tiếp nhận | Enum(String(50)) | Có | "Trực tiếp" | Chỉ đọc. Popup chỉ áp dụng hồ sơ có Nguồn tiếp nhận là "Trực tiếp". |
...
| 8 | Trả lại hồ sơ giấy | Chỉ áp dụng với hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp" ở trạng thái "Chờ ký". Khi Lãnh đạo trả lại, hồ sơ chuyển sang "Bị trả lại"; Cán bộ giải quyết được sửa tiêu chí/dữ liệu tra cứu, tra cứu lại, kết xuất PDF mới và trình ký lại theo [BR-CCTT-009]. Không áp dụng trả lại cho hồ sơ trực tuyến. |
...
| 14 | Lãnh đạo | Với hồ sơ giấy có Nguồn tiếp nhận là "Trực tiếp" ở trạng thái "Chờ ký", có thể chọn "Từ chối" hoặc "Trả lại" thay vì ký số nếu phát hiện nội dung chưa phù hợp. |
