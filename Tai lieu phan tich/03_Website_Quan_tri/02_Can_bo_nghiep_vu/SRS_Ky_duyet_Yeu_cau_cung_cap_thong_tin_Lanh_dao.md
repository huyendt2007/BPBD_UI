...
| Bảng danh sách hồ sơ | Text(1000) | Không | 20 bản ghi/trang | \- Chỉ hiển thị hồ sơ Yêu cầu cung cấp thông tin ở trạng thái "Chờ duyệt" thuộc phạm vi xử lý của Lãnh đạo đăng nhập.<br>\- Sắp xếp mặc định theo Thời điểm trình ký tăng dần để ưu tiên xử lý hồ sơ được trình trước.<br>\- Click trực tiếp vào dòng dữ liệu để mở **UC-CCTT-LD.MH02 - Chi tiết hồ sơ chờ duyệt**.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung: *"Không tìm thấy dữ liệu phù hợp với điều kiện tìm kiếm."*|
