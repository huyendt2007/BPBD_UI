...
| Danh sách hồ sơ trình ký | Table | Có | Theo hồ sơ được chọn | \- Chỉ hiển thị khi Chế độ trình ký là "Trình ký nhiều hồ sơ".<br>- Chỉ đọc.<br>- Hiển thị dạng bảng để Cán bộ đối chiếu trước khi xác nhận trình ký.<br>- Bảng gồm các cột:<br>+ STT.<br>+ Mã hồ sơ/Số đăng ký.<br>+ Loại đăng ký.<br>+ Loại hình giao dịch.<br>+ Người yêu cầu.<br>+ Trạng thái hồ sơ.<br>+ File PDF dự thảo.<br>+ Kết quả kiểm tra.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung: *"Không tìm thấy dữ liệu phù hợp với điều kiện tìm kiếm."* |
