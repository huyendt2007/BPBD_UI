...
| Loại thiệt hại khác tính được ngay | String(255) | Không | Trống | Control UI: Ô nhập văn bản (Input text).<br>- **Cơ chế**: Cho phép người dùng nhập trực tiếp bằng văn bản tên hoặc nội dung loại thiệt hại khác có thể tính được ngay (Ví dụ: *Viện phí khẩn cấp, Chi phí mai táng ban đầu, Thu nhập bị mất tính được ngay...*).<br>- **Lưu ý nghiệp vụ**: Tuyệt đối **không phải loại List chọn** (không dùng Combobox / Dropdown danh mục, không gò bó danh sách có sẵn). Bố cục giao diện hiển thị 01 ô Text để nhập tên loại thiệt hại và 01 ô nhập số tiền đi kèm.<br>- Chỉ hiển thị/nhập khi có tick chọn "Đề nghị tạm ứng kinh phí".<br>- Áp dụng ràng buộc đồng bộ: Nếu có nhập số tiền tạm ứng thiệt hại khác thì bắt buộc phải nhập tên loại thiệt hại tại ô text này. |
...
| Loại thiệt hại khác tính được ngay | String(255) | - | Theo dữ liệu | Chỉ đọc. Tên/nội dung loại thiệt hại khác tính được ngay do người yêu cầu nhập dạng văn bản (không phải danh sách List chọn). |
| Tạm ứng Thiệt hại khác tính được ngay (đồng) | Decimal(18,0) | - | Theo dữ liệu | Chỉ đọc. Số tiền đề nghị tạm ứng thiệt hại khác tính được ngay nếu có, định dạng phân tách hàng nghìn (VNĐ). Hiển thị kèm cặp với tên loại thiệt hại khác. |
