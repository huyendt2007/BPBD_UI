...
| **Bảng danh sách yêu cầu xác định cơ quan giải quyết bồi thường** | Text(4000) | - | 20 bản ghi/trang | Control UI: Data grid.<br>- Khi người dùng truy cập màn hình, hệ thống tự động tải trang đầu tiên (Trang 1) với số lượng mặc định 20 bản ghi.<br>- Sắp xếp mặc định: Sắp xếp theo "Thời điểm tiếp nhận" giảm dần (mới nhất hiển thị lên đầu).<br>- Cho phép click trực tiếp vào dòng dữ liệu để mở màn hình chi tiết, trừ khi click vào icon thao tác.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung: *"Không tìm thấy dữ liệu phù hợp với điều kiện tìm kiếm."* |
...
| 5 | Tiếp nhận | Icon button | Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ tiếp nhận`. Khi người dùng click icon, hệ thống mở **MH02 - Thêm mới/Chỉnh sửa yêu cầu xác định cơ quan giải quyết bồi thường** ở ngữ cảnh tiếp nhận hồ sơ liên thông từ bước **Tiếp nhận yêu cầu**; tiêu đề hiển thị `TIẾP NHẬN YÊU CẦU XÁC ĐỊNH CƠ QUAN GIẢI QUYẾT BỒI THƯỜNG`, tự động kế thừa dữ liệu tiếp nhận ban đầu và cho phép cán bộ chỉnh sửa/nhập bổ sung trước khi gửi yêu cầu. |
| 6 | Yêu cầu bổ sung | Icon button | Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ tiếp nhận`. Khi người dùng click icon, hệ thống mở **Popup Yêu cầu bổ sung / Từ chối yêu cầu** ở ngữ cảnh yêu cầu bổ sung để nhập nội dung yêu cầu bổ sung hồ sơ. |
| 7 | Từ chối | Icon button | Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ tiếp nhận`. Khi người dùng click icon, hệ thống mở **Popup Yêu cầu bổ sung / Từ chối yêu cầu** ở ngữ cảnh từ chối để nhập lý do từ chối yêu cầu. |
...
| 11 | Xóa yêu cầu | Icon button | Chỉ hiển thị và áp dụng với bản ghi ở trạng thái `Lưu nháp`. Khi người dùng click icon, hệ thống xử lý theo các trường hợp bên dưới. |
