...
\- **Lãnh đạo phê duyệt**: Xem xét hồ sơ; thực hiện phê duyệt hoặc từ chối kèm lý do đối với hồ sơ ở trạng thái `Chờ duyệt QĐ thành lập` (phê duyệt Quyết định thành lập Hội đồng) và trạng thái `Chờ duyệt QĐ hoàn trả` (phê duyệt dự thảo Quyết định hoàn trả).
...
| Trạng thái | Enum(String(50)) | Không | `Tất cả` | Control UI: Combobox.<br>Gồm:<br>+ Tất cả<br>+ Chờ thành lập hội đồng<br>+ Chờ duyệt QĐ thành lập<br>+ Bị từ chối thành lập HĐ<br>+ Đang họp hội đồng<br>+ Chờ ban hành QĐ hoàn trả<br>+ Chờ duyệt QĐ hoàn trả<br>+ Bị từ chối QĐ hoàn trả<br>+ Đang thi hành<br>+ Hoàn thành<br>+ Không xem xét trách nhiệm hoàn trả<br>+ Chấm dứt thu hồi |
| Xóa bộ lọc | Button | Không | - | Control UI: Button. |
| Tìm kiếm | Button | Không | - | Control UI: Button. |
| **III. Khối bảng kết quả tìm kiếm** | Section | - | 10 bản ghi/trang | Control UI: Data grid.<br>- Khi truy cập màn hình, hệ thống tự động tải toàn bộ hồ sơ hoàn trả thuộc phạm vi dữ liệu được phân quyền, sắp xếp mặc định theo ngày khởi tạo hồ sơ giảm dần.<br>- Riêng người dùng có quyền Lãnh đạo phê duyệt: mặc định áp dụng sẵn bộ lọc `Trạng thái` gồm `Chờ duyệt QĐ thành lập` và `Chờ duyệt QĐ hoàn trả`, sắp xếp theo `Hạn xử lý` tăng dần.<br>- Phân trang tuân thủ [BR-UI-001]. |
...
| Thao tác | Action | Không | - | Control UI: Nhóm nút thao tác.<br>Gồm các nút:<br>- **Cập nhật**: Chỉ hiển thị khi hồ sơ ở các trạng thái:<br>+ `Bị từ chối thành lập HĐ`<br>+ `Đang họp hội đồng`<br>+ `Bị từ chối QĐ hoàn trả`<br>+ `Đang thi hành`<br>- **Phê duyệt/Từ chối**:<br>+ Chỉ hiển thị khi người dùng được phân quyền Lãnh đạo phê duyệt.<br>+ Chỉ hiển thị khi hồ sơ ở trạng thái:<br>  * `Chờ duyệt QĐ thành lập`<br>  * `Chờ duyệt QĐ hoàn trả`<br>- **Xóa**: Chỉ hiển thị khi hồ sơ ở trạng thái:<br>+ `Chờ thành lập hội đồng` |
...
| Tạo QĐ hoàn trả | Button | Không | - | Control UI: Button primary.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ ban hành QĐ hoàn trả` (chưa lập quyết định). |
| **Khối Thông tin văn bản Quyết định hoàn trả** | Section | - | - | Control UI: Info card / Form card.<br>- **Điều kiện hiển thị**: Hiển thị khi người dùng click nút `Tạo QĐ hoàn trả` (tại trạng thái `Chờ ban hành QĐ hoàn trả`) hoặc khi hồ sơ ở các trạng thái:<br>+ `Chờ duyệt QĐ hoàn trả`<br>+ `Bị từ chối QĐ hoàn trả`<br>+ `Đang thi hành`<br>+ `Hoàn thành`<br>+ `Chấm dứt thu hồi`<br>+ `Không xem xét trách nhiệm hoàn trả`<br>- **Quy tắc thao tác**: Khi click nút Tạo QĐ hoàn trả hoặc trạng thái Bị từ chối QĐ hoàn trả khối này hiển thị ở chế độ nhập liệu. Đối với các trạng thái khác thì hiển thị dạng Chỉ đọc (Read-only). |
| Hình thức ban hành | Enum(String(50)) | Có | `Trình ký điện tử trên hệ thống` | Control UI: Radio button / Text.<br>Gồm:<br>+ `Trình ký điện tử trên hệ thống`<br>+ `Ký ngoài hệ thống`<br>- **Quy tắc thao tác**: Đối với trạng thái `Bị từ chối QĐ hoàn trả`, trường này ở chế độ chỉ đọc (Read-only), người dùng không được phép chỉnh sửa thay đổi hình thức ban hành. |
...
| Số quyết định | String(50) | Có | Theo dữ liệu | Control UI: Input text / Text (Read-only).<br>- Với hình thức `Ký ngoài hệ thống`: Cho phép nhập số quyết định chính thức.<br>- Với hình thức `Trình ký điện tử trên hệ thống`: Hiển thị `Chờ cấp số` khi hồ sơ ở trạng thái `Chờ ban hành QĐ hoàn trả`, `Chờ duyệt QĐ hoàn trả` hoặc `Bị từ chối QĐ hoàn trả`; tự động cấp số văn bản chính thức sau khi lãnh đạo ký duyệt thành công. |
...
| Thao tác trên dòng | Action | Không | - | Control UI: Nhóm nút thao tác trên từng dòng.<br>Gồm:<br>- `Sửa`: Mở khóa các ô nhập liệu trên dòng để chỉnh sửa trực tiếp.<br>- `Xóa`: Xóa dòng cán bộ khỏi danh sách quyết định hoàn trả.<br>- **Quy tắc hiển thị**: Chỉ hiển thị khi lập/sửa dự thảo hoặc khi trạng thái là `Bị từ chối QĐ hoàn trả`. |
| Trình ký | Button | - | - | Control UI: Button primary.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi Hình thức ban hành là `Trình ký điện tử trên hệ thống`, tại trạng thái `Chờ ban hành QĐ hoàn trả` (khi lập mới dự thảo) hoặc `Bị từ chối QĐ hoàn trả`. |
| Ban hành QĐ | Button | - | - | Control UI: Button success.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi Hình thức ban hành là `Ký ngoài hệ thống`, tại trạng thái `Chờ ban hành QĐ hoàn trả`. |
...
| 7 | Tạo QĐ hoàn trả | Button | Khi người dùng click nút Tạo QĐ hoàn trả (tại trạng thái `Chờ ban hành QĐ hoàn trả`), hệ thống ẩn Khối thông báo chưa ban hành và hiển thị `Khối Thông tin văn bản Quyết định hoàn trả` ở chế độ nhập liệu/chỉnh sửa để người dùng nhập thông tin ban hành, tải lên tệp văn bản dự thảo, bổ sung văn bản liên quan, điều chỉnh thông tin cán bộ chịu trách nhiệm hoàn trả và thực hiện Trình ký (nếu Trình ký điện tử) hoặc Ban hành QĐ (nếu Ký ngoài hệ thống). |
...
| Trạng thái | Enum(String(50)) | Có | Theo dữ liệu | Control UI: Badge (Read-only).<br>- Hiển thị trạng thái hiện tại của hồ sơ hoàn trả (chỉ đọc).<br>Gồm:<br>+ Chờ thành lập hội đồng<br>+ Chờ duyệt QĐ thành lập<br>+ Bị từ chối thành lập HĐ<br>+ Đang họp hội đồng<br>+ Chờ ban hành QĐ hoàn trả<br>+ Chờ duyệt QĐ hoàn trả<br>+ Bị từ chối QĐ hoàn trả<br>+ Đang thi hành<br>+ Hoàn thành<br>+ Không xem xét trách nhiệm hoàn trả<br>+ Chấm dứt thu hồi |
...
| Thời gian từ chối | DateTime | - | Theo dữ liệu | Control UI: Text (Read-only).<br>- Định dạng `dd/mm/yyyy - HH:mm`. Chỉ đọc. Theo thông tin hồ sơ. |
| Lý do từ chối | Text(1000) | - | Theo dữ liệu | Control UI: Text (Read-only).<br>- Chỉ đọc. Theo thông tin hồ sơ. |
| Khối thông báo chưa ban hành QĐ hoàn trả | Section | - | - | Control UI: Alert box / Call-to-action card.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ ban hành QĐ hoàn trả` nhưng chưa tạo quyết định hoàn trả. |
| Tiêu đề thông báo | String(255) | - | Theo dữ liệu | Control UI: Text heading (Read-only).<br>- Chỉ đọc. Theo thông tin hồ sơ (Hiển thị `Chưa ban hành Quyết định hoàn trả`). |
| Nội dung thông báo | Text(500) | - | Theo dữ liệu | Control UI: Text (Read-only).<br>- Chỉ đọc. Theo thông tin hồ sơ (Hiển thị `Hội đồng đã hoàn tất kiến nghị. Vui lòng tạo Quyết định hoàn trả để nhập thông tin ban hành và văn bản quyết định.`). |
| Tạo QĐ hoàn trả | Button | Không | - | Control UI: Button primary.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi hồ sơ ở trạng thái `Chờ ban hành QĐ hoàn trả` (chưa lập quyết định) và người dùng có quyền Cán bộ xử lý nghiệp vụ. |
| **Khối Thông tin văn bản Quyết định hoàn trả** | Section | - | - | Control UI: Info card / Form card.<br>- **Điều kiện hiển thị**: Hiển thị khi người dùng click nút `Tạo QĐ hoàn trả` (tại trạng thái `Chờ ban hành QĐ hoàn trả`) hoặc khi hồ sơ ở các trạng thái:<br>+ `Chờ duyệt QĐ hoàn trả`<br>+ `Bị từ chối QĐ hoàn trả`<br>+ `Đang thi hành`<br>+ `Hoàn thành`<br>+ `Chấm dứt thu hồi`<br>+ `Không xem xét trách nhiệm hoàn trả`<br>- **Quy tắc thao tác**: Khi click nút Tạo QĐ hoàn trả hoặc trạng thái Bị từ chối QĐ hoàn trả khối này hiển thị ở chế độ nhập liệu. Đối với các trạng thái khác thì hiển thị dạng Chỉ đọc (Read-only). |
| Hình thức ban hành | Enum(String(50)) | Có (khi nhập liệu) | `Trình ký điện tử trên hệ thống` | Control UI: Radio button / Text (Read-only).<br>Gồm:<br>+ `Trình ký điện tử trên hệ thống`<br>+ `Ký ngoài hệ thống`<br>- Cho phép chọn khi bấm Tạo QĐ hoàn trả; ở trạng thái `Bị từ chối QĐ hoàn trả` thì bị khóa chỉ đọc (Read-only); chỉ đọc ở các trạng thái khác. |
| Đơn vị ban hành | String(255) | Có | Đơn vị đang đăng nhập | Control UI: Combobox / Text (Read-only).<br>- Theo đơn vị ban hành quyết định (mặc định đơn vị đăng nhập). |
| Số quyết định | String(50) | Có | Theo dữ liệu | Control UI: Input text / Text (Read-only).<br>- Với hình thức `Ký ngoài hệ thống`: Cho phép nhập số quyết định chính thức.<br>- Với hình thức `Trình ký điện tử trên hệ thống`: Hiển thị `Chờ cấp số` khi hồ sơ ở trạng thái `Chờ ban hành QĐ hoàn trả`, `Chờ duyệt QĐ hoàn trả` hoặc `Bị từ chối QĐ hoàn trả`; tự động cấp số sau khi lãnh đạo ký duyệt thành công. |
| Ngày quyết định | Date | Có | Ngày hiện tại | Control UI: Datepicker `dd/mm/yyyy` / Text (Read-only).<br>- Ngày ký ban hành quyết định hoàn trả. Cho phép chọn khi lập quyết định; chỉ đọc khi đã ban hành. |
| Trích yếu | Text(2000) | Có | Trống | Control UI: Textarea / Text (Read-only).<br>- Trích yếu nội dung quyết định hoàn trả. Cho phép nhập/sửa khi lập quyết định hoặc khi `Bị từ chối QĐ hoàn trả`; chỉ đọc khi đã ban hành. |
| Lãnh đạo ký | Enum(String(255)) | Có (khi ký điện tử) | Trống | Control UI: Combobox / Text (Read-only).<br>- Chỉ hiển thị nếu Hình thức ban hành là `Trình ký điện tử trên hệ thống`. Cho phép chọn khi lập quyết định hoặc khi `Bị từ chối QĐ hoàn trả`; chỉ đọc khi đã ban hành. |
| Ngày ký | Date | Không | Theo dữ liệu | Control UI: Text (Read-only).<br>- Định dạng `dd/mm/yyyy`. Hiển thị ngày Lãnh đạo ký số phê duyệt Quyết định hoàn trả. Chỉ có giá trị sau khi Lãnh đạo ký duyệt thành công; trước đó hiển thị trống hoặc `Chưa ký`. |
| Tệp văn bản Quyết định | File (Multiple) | Có | Trống | Control UI: File upload / File link (Read-only).<br>- Cho phép tải lên tệp dự thảo (PDF) khi lập quyết định hoặc khi `Bị từ chối QĐ hoàn trả` (kèm liên kết Xem file và Xóa file). Hiển thị liên kết xem/tải khi đã ban hành (Read-only). |
| Bảng Văn bản liên quan | Section | Không | - | Control UI: Data grid.<br>- Cho phép đính kèm thêm văn bản liên quan hoặc xóa văn bản khi lập quyết định hoặc trạng thái `Bị từ chối QĐ hoàn trả`. Chỉ đọc khi đã ban hành. |
| Bảng danh sách cán bộ chịu trách nhiệm hoàn trả | Section | Có | - | Control UI: Data grid.<br>- Kế thừa từ Bước 2, cho phép chỉnh sửa lại thông tin cán bộ (Sửa, Xóa) khi lập dự thảo Quyết định hoàn trả hoặc khi `Bị từ chối QĐ hoàn trả`. Khi đã ban hành chính thức: Toàn bộ thông tin trên bảng là chỉ đọc (Read-only). |
| STT | Integer | - | Theo dữ liệu | Control UI: Text (Read-only).<br>- Số thứ tự tự động tăng (1, 2, 3...). |
| Cán bộ gây sai phạm | String(255) | Có | Theo dữ liệu | Control UI: Input text / Text (Read-only).<br>- Cho phép chỉnh sửa khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Chức vụ | String(255) | Có | Theo dữ liệu | Control UI: Input text / Text (Read-only).<br>- Cho phép chỉnh sửa khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Đơn vị công tác | String(255) | Có | Theo dữ liệu | Control UI: Combobox (hỗ trợ tìm kiếm) / Text (Read-only).<br>- Cho phép chọn/sửa khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Mức độ lỗi | Enum(String(50)) | Có | Theo dữ liệu | Control UI: Combobox / Badge (Read-only).<br>Gồm:<br>+ `Cố ý`<br>+ `Vô ý`<br>- Cho phép chọn lại khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Số tiền hoàn trả (VNĐ) | Decimal(18,0) | Có | Theo dữ liệu | Control UI: Input number / Text (Read-only).<br>- Căn phải, định dạng phân cách hàng nghìn. Cho phép sửa khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Phương thức hoàn trả | Enum(String(50)) | Có | Theo dữ liệu | Control UI: Combobox / Text (Read-only).<br>Gồm:<br>+ `Một lần`<br>+ `Nhiều lần`<br>- Cho phép chọn lại khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Trạng thái hoãn | Enum(String(50)) | Có | Theo dữ liệu | Control UI: Combobox / Badge (Read-only).<br>Gồm:<br>+ `Không hoãn`<br>+ `Đề xuất hoãn`<br>- Cho phép chọn lại khi lập dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Thao tác trên dòng | Action | Không | - | Control UI: Nhóm nút thao tác trên từng dòng (Sửa, Xóa).<br>- Chỉ hiển thị khi lập/sửa dự thảo hoặc khi `Bị từ chối QĐ hoàn trả`. |
| Trình ký | Button | - | - | Control UI: Button primary.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi Hình thức ban hành là `Trình ký điện tử trên hệ thống`, tại trạng thái `Chờ ban hành QĐ hoàn trả` (khi tạo QĐ) hoặc `Bị từ chối QĐ hoàn trả`. |
| Ban hành QĐ | Button | - | - | Control UI: Button success.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi Hình thức ban hành là `Ký ngoài hệ thống`, tại trạng thái `Chờ ban hành QĐ hoàn trả`. |
...
| **IX. Khối thanh tác vụ cố định chân trang (Sticky Footer Actions)** | Section | - | - | Vùng nút tác nghiệp cố định phía dưới màn hình xem chi tiết, hiển thị động theo trạng thái hồ sơ và phân quyền người dùng. |
| Đóng | Button | - | - | Control UI: Button secondary.<br>- Luôn hiển thị ở mọi trạng thái hồ sơ để đóng màn hình chi tiết. |
| Phê duyệt / Từ chối | Button | - | - | Control UI: Button success / danger.<br>- Chỉ hiển thị khi người dùng được phân quyền Lãnh đạo phê duyệt và hồ sơ ở trạng thái `Chờ duyệt QĐ thành lập` hoặc `Chờ duyệt QĐ hoàn trả`. |
| Cập nhật | Button | - | - | Control UI: Button primary.<br>- **Điều kiện hiển thị**: Chỉ hiển thị khi hồ sơ ở các trạng thái:<br>+ `Bị từ chối thành lập HĐ`<br>+ `Đang họp hội đồng`<br>+ `Bị từ chối QĐ hoàn trả`. |
| Cập nhật hoàn trả | Button | - | - | Control UI: Button primary.<br>- Chỉ hiển thị khi hồ sơ ở trạng thái `Đang thi hành`. |
...
| Trường thông tin | Kiểu dữ liệu | Bắt buộc | Mặc định | Mô tả |
| :--- | :--- | :--- | :--- | :--- |
| Tiêu đề popup | String(255) | - | - | Control UI: Text heading (Read-only).<br>- Khi hồ sơ ở trạng thái `Chờ duyệt QĐ thành lập`: Hiển thị `Ký số phê duyệt Quyết định thành lập Hội đồng`.<br>- Khi hồ sơ ở trạng thái `Chờ duyệt QĐ hoàn trả`: Hiển thị `Ký số phê duyệt Quyết định hoàn trả`. |
...
| Trường thông tin | Kiểu dữ liệu | Bắt buộc | Mặc định | Mô tả |
| :--- | :--- | :--- | :--- | :--- |
| Tiêu đề popup | String(255) | - | - | Control UI: Text heading (Read-only).<br>- Khi hồ sơ ở trạng thái `Chờ duyệt QĐ thành lập`: Hiển thị `Từ chối phê duyệt Quyết định thành lập Hội đồng`.<br>- Khi hồ sơ ở trạng thái `Chờ duyệt QĐ hoàn trả`: Hiển thị `Từ chối phê duyệt dự thảo Quyết định hoàn trả`. |
