...
| Loại khách hàng | Enum(String(50)) | Không | Theo hồ sơ tài khoản | - Control UI: Text hiển thị (Read-only).<br>- Giá trị: `Cá nhân` hoặc `Tổ chức`.<br>- Quyền chỉnh sửa: Luôn chỉ đọc. |
| Loại tài khoản | Enum(String(50)) | Không | Theo người dùng đang đăng nhập | - Control UI: Text hiển thị (Read-only).<br>- Giá trị: `Tài khoản chính` hoặc `Tài khoản phụ`.<br>- Quyền chỉnh sửa: Luôn chỉ đọc. |
| Nguồn xác thực | Enum(String(100)) | Không | Theo hồ sơ tài khoản | - Control UI: Text hiển thị (Read-only).<br>- Giá trị: `VNeID`, `Nội bộ` hoặc `VNeID, Nội bộ`.<br>- Quyền chỉnh sửa: Luôn chỉ đọc. |
| **II.A. Thông tin khách hàng Cá nhân** | - | - | - | - Khối chỉ hiển thị khi `Loại khách hàng = Cá nhân`. |
| Loại giấy tờ | Enum(String(50)) | Có | Theo hồ sơ tài khoản | - Control UI: Combobox.<br>- Danh mục: Tham chiếu [DM_10], gồm các giá trị `CCCD`, `CMND`, `Hộ chiếu`.<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi `Nguồn xác thực = VNeID` hoặc `VNeID, Nội bộ`.<br>- Ràng buộc: Khi sửa, kiểm tra trùng lặp theo bộ thông tin định danh cá nhân. |
| Số giấy tờ | String(50) | Có | Theo hồ sơ tài khoản | - Control UI: Input text.<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi `Nguồn xác thực = VNeID` hoặc `VNeID, Nội bộ`.<br>- Ràng buộc: Kiểm tra trùng số giấy tờ trên toàn hệ thống [BR-VAL-009], ngoại trừ hồ sơ hiện tại. |
...
| Quốc tịch | Enum(String(255)) | Có | Theo hồ sơ tài khoản | - Control UI: Combobox.<br>- Danh mục: Tham chiếu [DM_09].<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi `Nguồn xác thực = VNeID` hoặc `VNeID, Nội bộ`.<br>- Tác vụ khi thay đổi: Tự động cập nhật `Phân loại khách hàng` và chuyển đổi cách nhập của trường `Tỉnh/Thành phố`. |
| Email cá nhân (Tên đăng nhập) | String(255) | Có nếu hồ sơ có Nội bộ | Theo hồ sơ tài khoản | - Control UI: Input text.<br>- Quyền chỉnh sửa: Cho phép sửa khi hồ sơ chỉ có `VNeID` và đóng vai trò email liên hệ; khóa chỉ đọc khi có `Nội bộ` hoặc `VNeID, Nội bộ` vì đóng vai trò tên đăng nhập hệ thống.<br>- Ràng buộc: Kiểm tra định dạng email theo [BR-VAL-002]. |
...
| Mã định danh tổ chức / Mã số thuế/Số giấy phép đầu tư | String(100) | Có theo loại tổ chức | Theo hồ sơ tài khoản | - Control UI: Input text.<br>- Hiển thị khi: Nhãn và ô nhập tự động đổi theo `Loại tổ chức`.<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi có `VNeID`.<br>- Ràng buộc: Kiểm tra trùng lặp trên toàn hệ thống [BR-VAL-009], ngoại trừ hồ sơ hiện tại. |
...
| Quốc gia đăng ký | Enum(String(255)) | Có | Theo hồ sơ tài khoản | - Control UI: Combobox.<br>- Danh mục: Tham chiếu [DM_09].<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi `Nguồn xác thực = VNeID` hoặc `VNeID, Nội bộ`.<br>- Tác vụ khi thay đổi: Tự động cập nhật `Phân loại khách hàng` và chuyển đổi cách nhập của trường `Tỉnh/Thành phố`. |
| Email tổ chức (Tên đăng nhập) | String(255) | Có nếu hồ sơ có Nội bộ | Theo hồ sơ tài khoản | - Control UI: Input text.<br>- Quyền chỉnh sửa: Cho phép sửa khi hồ sơ chỉ có `VNeID`; khóa chỉ đọc khi có `Nội bộ` hoặc `VNeID, Nội bộ`.<br>- Ràng buộc: Kiểm tra định dạng email theo [BR-VAL-002]. |
...
| Loại giấy tờ người đại diện | Enum(String(50)) | Không | Theo hồ sơ tài khoản | - Control UI: Combobox.<br>- Danh mục: Tham chiếu [DM_10]. Giá trị gồm `CCCD`, `CMND`, `Hộ chiếu`.<br>- Quyền chỉnh sửa: Cho phép sửa khi `Nguồn xác thực = Nội bộ`; khóa chỉ đọc khi có `VNeID`. |
...
| Trung tâm Đăng ký mặc định | Enum(String(255)) | Không | Theo hồ sơ tài khoản | - Control UI: Combobox.<br>- Danh mục: Tham chiếu [DM_08].<br>- Quyền chỉnh sửa: Luôn cho phép chỉnh sửa. |
...
| Tỉnh/Thành phố | Enum/String(255) | Có | Theo hồ sơ tài khoản | - Control UI: Combobox khi trong nước, tham chiếu [DM_13]; Input text khi nước ngoài.<br>- Quyền chỉnh sửa: Luôn cho phép chỉnh sửa. |
...
| Điều kiện | Khối thông tin hiển thị |
| :--- | :--- |
| `Loại khách hàng = Cá nhân` | Khối I, Khối II.A, Khối III |
| `Loại khách hàng = Tổ chức` | Khối I, Khối II.B, Khối III |

##### 4.1.8.3.4. Chức năng trên màn hình
...
| | | | **TH3 - Email đã tồn tại**: Nếu Email được phép sửa và trùng với email đã đăng ký của tài khoản khác trên hệ thống, vi phạm [BR-VAL-009], hệ thống hiển thị thông báo lỗi [MSG-ERR-VAL-009]. Không cho phép lưu. |
| | | | **TH4 - Số điện thoại không hợp lệ**: Nếu Số điện thoại nhập sai định dạng hoặc trùng với tài khoản khác đang hoạt động trên hệ thống, vi phạm [BR-VAL-009], hệ thống hiển thị thông báo lỗi tương ứng. Không cho phép lưu. |
| | | | **TH5 - Số giấy tờ/Mã định danh trùng**: Nếu trường định danh được phép sửa nhưng bị trùng với hồ sơ tài khoản khác trên toàn hệ thống, ngoại trừ hồ sơ hiện tại, vi phạm [BR-VAL-009], hệ thống hiển thị thông báo lỗi dưới ô tương ứng. Không cho phép lưu. |
