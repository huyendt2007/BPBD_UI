...
| Tiêu chí yêu cầu cung cấp thông tin | Enum(String(50)) | Có | "Số đăng ký" | - Control UI: Tab/Segmented control.<br>- Chọn một trong các giá trị:<br>+ Số đăng ký<br>+ Bên bảo đảm<br>+ Số khung<br>- Khi chuyển qua lại giữa các Tab/tiêu chí, hệ thống giữ nguyên toàn bộ dữ liệu người dùng đã nhập ở từng Tab và chỉ ẩn/hiện nhóm trường tương ứng với Tab đang active.<br>- Khi bấm `Gửi yêu cầu`, hệ thống chỉ lấy tiêu chí và dữ liệu của Tab đang active để kiểm tra, tính phí và lưu hồ sơ; dữ liệu ở các Tab đang ẩn không được lưu vào hồ sơ. |
...
| Loại chủ thể | Enum(String(50)) | Tùy điều kiện | "Công dân Việt Nam" | - Chỉ hiển thị và bắt buộc nếu Tiêu chí yêu cầu cung cấp thông tin là "Bên bảo đảm".<br>- Tham chiếu Danh mục Loại bên bảo đảm (Chủ thể) [DM_06]. |
...
| :--- | :--- | :--- | :--- |
| 1 | Thay đổi tiêu chí | Tab/Segment | **TH1 - Chuyển Tab có dữ liệu đã nhập**: Hệ thống chuyển sang Tab được chọn, ẩn nhóm trường của Tab cũ, hiển thị nhóm trường của Tab mới và giữ nguyên toàn bộ dữ liệu đã nhập ở từng Tab trong phiên làm việc hiện tại. |
|  |  |  | **TH2 - Gửi yêu cầu sau khi chuyển Tab**: Hệ thống chỉ lấy tiêu chí và dữ liệu của Tab đang active để kiểm tra bắt buộc, kiểm tra định dạng, tính phí và lưu hồ sơ; không lấy/lưu dữ liệu của các Tab đang ẩn. |
...
|  |  |  | TH Hợp lệ: Hệ thống thực hiện:<br>+ Lưu hồ sơ Yêu cầu cung cấp thông tin vào CSDL.<br>+ Sinh Mã hồ sơ theo cấu trúc `CCTT-[YYYYMMDD]-[TỰ_TĂNG_6_SỐ]`.<br>+ Ghi nhận Người yêu cầu từ tài khoản đăng nhập, Thời điểm đăng ký, Nguồn tiếp nhận là Website Khách hàng.<br>+ Chỉ lưu tiêu chí và dữ liệu tra cứu của Tab đang active tại thời điểm bấm `Gửi yêu cầu`; không lưu đồng thời dữ liệu của các Tab đang ẩn.<br>+ Xác định nghĩa vụ phí theo biểu phí cung cấp thông tin đang có hiệu lực tại thời điểm phát sinh hồ sơ.<br>+ Nếu hồ sơ thuộc diện phải thu phí, hệ thống tạo hồ sơ ở trạng thái "Chờ thanh toán", đóng gói tham số `Mã loại thanh toán = THANH_TOAN_CCTT`, `Mã hồ sơ`, `Số tiền phải thu`, `Mã đơn vị thụ hưởng`, `Return URL` và chuyển sang UC158 theo tài liệu [UC158_Thanh_toan_truc_tuyen.md](UC158_Thanh_toan_truc_tuyen.md).<br>+ Nếu hồ sơ thuộc diện miễn phí, hệ thống tạo hồ sơ ở trạng thái "Chờ duyệt" và điều hướng sang **UC141.MH03 - Thông báo gửi hồ sơ thành công**.<br>+ Ghi nhật ký thao tác gửi yêu cầu vào III.6. |
...
| Nội dung hướng dẫn | Text(1000) | Có | Theo hệ thống | Chỉ đọc. Hiển thị thông báo chung hồ sơ đã được gửi thành công và hướng dẫn Khách hàng theo dõi trạng thái tại danh sách yêu cầu đã gửi. Màn hình này không mô tả chi tiết hướng dẫn thanh toán; nếu hồ sơ thuộc diện phải thu phí, hệ thống chuyển tiếp luồng thanh toán sang UC158 theo tài liệu [UC158_Thanh_toan_truc_tuyen.md](UC158_Thanh_toan_truc_tuyen.md). |

---

#### 4.1.1.6. Tham chiếu phân hệ thanh toán trực tuyến UC158

Tài liệu CCTT không đặc tả chi tiết quy tắc thanh toán, màn hình thanh toán, callback, kết quả thanh toán hoặc phát hành biên lai.

Khi hồ sơ Yêu cầu cung cấp thông tin phát sinh nghĩa vụ phí, hệ thống chuyển hướng sang phân hệ thanh toán trực tuyến theo tài liệu [UC158_Thanh_toan_truc_tuyen.md](UC158_Thanh_toan_truc_tuyen.md), đồng thời truyền đủ bộ tham số đóng gói sau:
...
| Mã hồ sơ | String(50) | Có | Theo hồ sơ | Control UI: Hidden/System parameter.<br>- Mã hồ sơ Yêu cầu cung cấp thông tin được tạo trên hệ thống. |
...
| Return URL | String(500) | Có | Theo hệ thống | Control UI: Hidden/System parameter.<br>- Đường dẫn quay lại Website Khách hàng sau khi UC158 xử lý xong giao dịch. |
