...
| **II. Bảng danh sách hồ sơ** | - | - | 20 bản ghi/trang | Control UI: Grid phẳng.<br>- Click trực tiếp vào dòng dữ liệu để mở màn hình chi tiết, ngoại trừ khi click nút tại cột Thao tác.<br>- Trạng thái có dữ liệu: Hiển thị danh sách các bản ghi kết quả theo cấu trúc các cột quy định.<br>- Trạng thái không có dữ liệu (Empty State): Khi không tìm thấy kết quả phù hợp với điều kiện tìm kiếm, bảng hiển thị duy nhất 01 dòng căn giữa trên toàn bộ chiều rộng bảng (`colspan`), in nghiêng với nội dung theo MessageList dùng chung [MSG-INF-SYS-001].|
...
| 4 | Thanh toán | Button | Khi click, hệ thống đóng gói thông tin thanh toán (Mã hồ sơ, Số tiền phải thu, Mã đơn vị thụ hưởng, Return URL) và chuyển hướng sang cổng thanh toán trực tuyến [UC158 - Quản lý thanh toán phí](UC158_Thanh_toan_truc_tuyen.md). |
...
- Các khối xử lý/thanh toán/kết quả hiển thị động theo trạng thái hồ sơ. Toàn bộ các nút hành động của màn hình (Quay lại, Thanh toán, Xem file, Tải file, Xem/In biên lai) được bố trí thống nhất tại khu vực Header (góc trên bên phải màn hình).
...
| Trạng thái thanh toán | Enum(String(50)) | Không | Theo hồ sơ | Hiển thị trạng thái thanh toán của hồ sơ: `Chưa thanh toán`, `Đã thanh toán`, `Miễn phí`... |
...
| Danh sách hồ sơ liên quan | Grid/Khối thông tin | Không | Theo kết quả tra cứu | Hiển thị danh sách các bản ghi hồ sơ liên quan từ kết quả thực hiện chức năng Cung cấp thông tin (chi tiết hiển thị và cấu trúc hồ sơ kế thừa theo quy định tại chức năng Cung cấp thông tin). Không cho phép chỉnh sửa. |
...
| 2 | Thanh toán | Button | Bố trí tại Header góc trên bên phải màn hình (hiển thị khi hồ sơ ở trạng thái `Chờ thanh toán`). Khi click: Thực hiện thanh toán trực tuyến cho giao dịch cung cấp thông tin qua phân hệ UC158. |
