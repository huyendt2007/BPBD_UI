...
- Tài liệu này chỉ mô tả Tab Phiếu đăng ký, gồm:<br>+ Tab Phiếu đăng ký.<br>+ Xem chi tiết Phiếu đăng ký.<br>+ Các thao tác nghiệp vụ phát sinh từ Phiếu đăng ký.
...
![Tab Phiếu đăng ký](images/MH01_Tab_Phieu_dang_ky.png)
...
| Trạng thái | Enum(String(50)) | Không | Tất cả | Control UI: Combobox.<br>Gồm:<br>+ Tất cả<br>+ Lưu nháp<br>+ Chờ thanh toán<br>+ Chờ duyệt<br>+ Chờ ký<br>+ Hoàn thành<br>+ Bị từ chối<br>+ Sai lệch thanh toán |
...
| Loại hình giao dịch | Enum(String(100)) | Không | Tất cả | Control UI: Combobox.<br>Gồm:<br>+ Tất cả<br>+ Biện pháp bảo đảm<br>+ Hợp đồng<br>+ Thông báo xử lý tài sản bảo đảm |
| Loại biện pháp | Enum(String(100)) | Không | Tất cả | Control UI: Combobox.<br>- Lọc theo loại biện pháp bảo đảm.<br>- Chỉ hiển thị/cho phép chọn khi trường `Loại hình giao dịch` chọn "Biện pháp bảo đảm".<br>- Tham chiếu Danh mục Loại biện pháp bảo đảm [DM_04]. |
| Loại hợp đồng | Enum(String(100)) | Không | Tất cả | Control UI: Combobox.<br>- Lọc theo loại hợp đồng.<br>- Chỉ hiển thị/cho phép chọn khi trường `Loại hình giao dịch` chọn "Hợp đồng".<br>- Tham chiếu Danh mục Loại hợp đồng [DM_05]. |
...
| Loại hình giao dịch | String(100) | - | - | Hiển thị loại hình giao dịch. |
| Loại biện pháp | String(100) | - | - | Hiển thị loại Biện pháp bảo đảm của phiên bản (nếu có). |
| Loại hợp đồng | String(100) | - | - | Hiển thị loại Hợp đồng của phiên bản (nếu có). |
...
| 3 | Rowclick | Row Click | Khi NSD click trực tiếp vào một dòng dữ liệu trên Tree Grid, hệ thống mở màn hình **Phiếu đăng ký - Xem chi tiết** và hiển thị đúng phiên bản tương ứng với dòng được click. |
| 4 | Thanh toán | Button/Icon | Khi NSD click nút `Thanh toán`, hệ thống chuyển sang phân hệ Thanh toán trực tuyến. |
...
| | | | **TH Đăng ký thay đổi**: Hệ thống chuyển sang màn hình **Đăng ký thay đổi biện pháp bảo đảm**, kế thừa toàn bộ thông tin hồ sơ gốc gồm `Số đăng ký`, `Số PIN`, thông tin bên bảo đảm, bên nhận bảo đảm, tài sản bảo đảm và các thông tin liên quan; hệ thống đưa NSD tới bước nhập nội dung thay đổi mà không yêu cầu nhập lại số đăng ký/số PIN. |
| | | | **TH Thông báo xử lý tài sản**: Hệ thống chuyển sang màn hình **Thông báo xử lý tài sản bảo đảm lần đầu**, kế thừa toàn bộ thông tin hồ sơ gốc gồm `Số đăng ký`, `Số PIN`, thông tin bên bảo đảm, bên nhận bảo đảm, tài sản bảo đảm và các thông tin liên quan; hệ thống mặc định `Nguồn tài sản thông báo xử lý` là `Từ hồ sơ BPBĐ đã đăng ký` và đưa NSD tới màn nhập liệu tương ứng để khai báo thông tin xử lý tài sản. |
| | | | **TH Xóa đăng ký**: Hệ thống chuyển sang màn hình **Xóa đăng ký biện pháp bảo đảm**, kế thừa thông tin hồ sơ gốc gồm `Số đăng ký`, `Số PIN`, thông tin bên bảo đảm, bên nhận bảo đảm, tài sản bảo đảm và các thông tin liên quan; hệ thống đưa NSD tới bước xác nhận yêu cầu xóa đăng ký. |
| | | | **TH Thay đổi thông báo**: Hệ thống chuyển sang màn hình **Thay đổi thông báo xử lý tài sản bảo đảm**, kế thừa thông tin của thông báo xử lý tài sản bảo đảm đang có hiệu lực gần nhất; hệ thống đưa NSD tới form cập nhật nội dung thay đổi thông báo. |
| | | | **TH Xóa thông báo**: Hệ thống chuyển sang màn hình **Xóa đăng ký thông báo xử lý tài sản bảo đảm**, kế thừa thông tin của thông báo xử lý tài sản bảo đảm đang có hiệu lực để NSD lập yêu cầu xóa thông báo. |
...
| Trạng thái hồ sơ | Enum(String(50)) | Có | Theo phiên bản | Control UI: Badge/Text trạng thái.<br>Gồm:<br>+ Lưu nháp<br>+ Chờ thanh toán<br>+ Chờ duyệt<br>+ Chờ ký<br>+ Hoàn thành<br>+ Bị từ chối<br>+ Sai lệch thanh toán |
...
| 9 | Thanh toán | Button | Khi NSD click nút `Thanh toán`, hệ thống chuyển sang phân hệ Thanh toán trực tuyến. |
| 10 | Thao tác khác | Dropdown | Thực hiện theo chức năng `Thao tác khác` tại mục [4.1.12.2.3](#411223-chuc-nang-tren-man-hinh). |
| 11 | Đóng | Button | Đóng màn hình và quay lại Tab Phiếu đăng ký. |
